...
def add_dummy_feature():
    ...
    b = preprocessing.add_dummy_feature(a)      # 1 붙이기
    print(b)
...
def binarizer():
    x = [[-1, 3, 2],
         [5, -7, -4]]
    print(preprocessing.binarize(x))        # 양수: 1, 음수: 0

    bin = preprocessing.Binarizer(threshold=-2)     # -2 이하로, 기본값은 0
    bin.fit(x)
    print(bin.transform(x))


def label_binarizer():
    x = [1, 2, 6, 2, 4]     # 1, 2, 4, 6을 클래스라 한다.
    ...
    print(lb1.classes_[np.argmax(y, axis=1)])       # 변환

    print(lb1.inverse_transform(y))                 # 역변환
    print('-' * 30)
