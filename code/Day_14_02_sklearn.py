...
def predict_2():
    ...
    y_hats = clf.predict(digits['data'])
    equals = (y_hats == digits['target'])
    print('acc :', np.mean(equals))


# 문제
# 마지막 1개를 제외한 데이터로 학습하고 마지막 1개에 대해 예측하세요
def predict_3():
    digits = datasets.load_digits()
    ...
    print(y_hats, digits['target'][-1:])


# 문제
# 70%의 데이터로 학습하고 30% 데이터에 대해 예측하세요.
def predict_4():
    digits = datasets.load_digits()
